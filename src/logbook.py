...
def save_evidence(img: np.ndarray | None, reason: str) -> Path | None:
    ...
    try:
        with _lock:
            ...
            slug = re.sub(r"[^a-z0-9]+", "-", reason.lower()).strip("-")[:40] or "evidencia"
            path = _evidence_dir / f"{datetime.now():%Y%m%d-%H%M%S}_{slug}.png"
            cv2.imwrite(str(path), img)
            old = sorted(_evidence_dir.glob("*.png"))
            for extra in old[:-MAX_EVIDENCE]:
